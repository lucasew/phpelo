...
**- Justification:** The project maintains a minimal environment footprint. The build system (Nix) manages dependencies and environment, and adding these files is considered unnecessary noise (PRs #24, #27, #30).
...
## IGNORE: Fixing LFI / Path Traversal Prefix Bypass in execphp

**- Pattern:** Do not attempt to fix the LFI / Path Traversal vulnerability in `execphp` related to `str_starts_with` prefix-matching bypass by appending `DIRECTORY_SEPARATOR` to the base path.
**- Justification:** Multiple attempts to fix this specific bypass (PRs #38-48) have been rejected. The current implementation is considered acceptable, or changes here are actively rejected.
**- Files Affected:** `entrypoint.php`
