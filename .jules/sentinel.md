...
## 2026-03-01 - LFI / Path Traversal Bypass via Prefix Matching
**Vulnerability:** The `execphp()` function attempted to prevent Local File Inclusion (LFI) by using `str_starts_with($real_script_path, $base_path)` to check if the script resolved to the current working directory. This implementation was vulnerable to prefix matching. If the `$base_path` was `/app/foo`, a path like `/app/foo2/script.php` would pass the check, allowing an attacker to escape the intended directory and execute scripts from a directory with a partially matching name.
**Learning:** Using `str_starts_with` or similar string prefix checks for path containment is inherently flawed unless a path separator (like `/` or `DIRECTORY_SEPARATOR`) is appended to the base path. This is a common and subtle pitfall when implementing custom path traversal protections.
**Prevention:** When validating that a path resides within a specific directory, ensure the base directory ends with a directory separator before checking if the resolved path starts with it (e.g., `$base_path . DIRECTORY_SEPARATOR`). Alternatively, use dedicated path manipulation libraries or functions that are aware of directory hierarchies if available.
