...
## 2026-02-14 - Path Traversal Prefix Bypass
**Vulnerability:** The `execphp()` function used `str_starts_with($real_script_path, $base_path)` to prevent Local File Inclusion (LFI). This check can be bypassed if an attacker can control part of the path and accesses a sibling directory that shares the same prefix (e.g., if `$base_path` is `/app/examples`, the attacker might access `/app/examples_private/test.php`).
**Learning:** Simple string prefix checking is insufficient for verifying directory containment because it matches arbitrary directory names that happen to start with the same characters.
**Prevention:** Always append a trailing directory separator (e.g., `DIRECTORY_SEPARATOR`) to the base path before performing string-based path containment checks.
