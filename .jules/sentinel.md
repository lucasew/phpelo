...
## 2026-01-25 - Local File Inclusion (LFI) via Path Traversal Prefix Bypass
**Vulnerability:** The `execphp()` function attempted to prevent LFI by ensuring the resolved script path started with the current working directory (`str_starts_with($real_script_path, $base_path)`). However, if `$base_path` was `/app/examples`, an attacker could request `/app/examples2/malicious.php`, which would bypass the check because the string started with the required prefix, despite being a different directory.
**Learning:** Using basic string prefix matching (`str_starts_with` or `strpos`) for directory boundary checks is insufficient and a common vulnerability pattern. It allows an attacker to break out of the intended directory by accessing a sibling directory with a name that shares the same prefix.
**Prevention:** Always append a directory separator (`/` or `DIRECTORY_SEPARATOR`) to the base path before performing prefix matching to ensure exact directory boundary enforcement (e.g., `rtrim($base_path, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR`).
