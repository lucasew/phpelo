...
## 2026-01-16 - HTTP Response Splitting via Header Injection
**Vulnerability:** The custom `header()` and `set_header()` functions in `entrypoint.php` lacked validation for Carriage Return (CR) and Line Feed (LF) characters. This allowed potential HTTP Response Splitting (CRLF injection) if user input was passed directly to these functions by route handlers.
**Learning:** Unlike PHP's native `header()` function (since 5.1.2), custom implementations of HTTP response handling do not automatically sanitize headers. Developers must manually implement safeguards against control character injection to prevent headers from terminating early and injecting arbitrary content (or new requests/responses).
**Prevention:** Strictly validate all header keys and values for CRLF characters (`\r`, `\n`) before appending them to the response buffer. Fail securely by rejecting the operation if invalid characters are detected.
