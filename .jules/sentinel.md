...
## 2026-01-20 - Custom Header Implementation Lacks CRLF Protection
**Vulnerability:** The custom `header()` and `set_header()` functions in `entrypoint.php` directly echoed user input to the HTTP response stream without validation. This allowed attackers to inject Carriage Return and Line Feed (`\r\n`) characters, leading to HTTP Response Splitting.
**Learning:** Custom re-implementations of standard library functions (like PHP's `header()`) often bypass the built-in security protections provided by the language runtime. Developers must explicitly re-implement these protections (e.g., validating against control characters).
**Prevention:** When overriding or re-implementing core I/O functions, always audit the original implementation's security guarantees and ensure they are preserved in the new code. Specifically for HTTP headers, always validate input against `\r` and `\n`.
