...
$_HEADERS = array();
function _header(string $header) {
    global $_HEADERS;
...
function content_scope_pop_markdown() {
    ...
    // 🛡️ Sentinel: Fix Stored XSS.
    // Escape all input first, then apply markdown formatting.
    ...
    while (true) {
        ...
        content_scope_push();
        // Sentinel: Removed redundant htmlspecialchars
    ...
    foreach ($lines as $i => $line) {
        ...
        if ($heading_level > 0) {
            ...
            // Sentinel: Adapted regex to handle escaped '>'
            preg_match('/^(&gt;)*/', $line, $matches);
            $bq_len = strlen($matches[0]);
            if ($bq_len > 0) {
                // Each &gt; is 4 chars
                $blockquote_level = $bq_len / 4;
                $line = substr($line, $bq_len);
                $line = trim($line);
